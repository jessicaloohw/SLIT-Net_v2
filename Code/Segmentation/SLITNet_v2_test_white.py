...
def main():
    ...
    for K_FOLD in ['K1','K2','K3','K4','K5','K6']:
        ...
        for image_id in range(NUM_IMAGES):
            ...
            results["class_ids"] = np.concatenate((results["class_ids"], [7]))
            ...
            # Clean predictions:
            results = restrict_within_limbus(results, IDS_TO_RESTRICT=[1, 2, 4, 6], HIGH_OVERLAP=0.7, LIMBUS_ID=7)
            results = restrict_within_limbus(results, IDS_TO_RESTRICT=[3], HIGH_OVERLAP=0.5, LIMBUS_ID=7)
            ...
            # Limbus length:
            truth_limbus_length = get_limbus_length(ground_truth, limbus_id=7)
            pred_limbus_length = get_limbus_length(results, limbus_id=7)
