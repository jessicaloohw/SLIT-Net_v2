...
def main():
    ...
    for image_id in val_dataset.image_ids:
        ...
        for class_num in range(1, NUM_CLASSES+1):
            ...
            for threshold_idx, threshold_val in enumerate(THRESHOLD_RANGE):
                ...
                # Initialise thresholds for each class:
                thresholds = np.zeros([NUM_CLASSES, 1])
                thresholds[class_num-1, 0] = threshold_val
                ...
                results = get_predictions(results, THRESHOLD_MASK)
                results = restrict_within_limbus(results, IDS_TO_RESTRICT=[1, 3], HIGH_OVERLAP=0.7, LIMBUS_MASK=limbus_mask)
    ...
    for class_num in range(1, NUM_CLASSES+1):
        SUMMARY_FILENAME = initialize_validation_summary_metrics(SAVE_FOLDER, 'THRESHOLD_CLASS', class_num)
    ...
    for class_num in range(1, NUM_CLASSES+1):
        best_threshold = plot_validation_summary_metrics(SAVE_FOLDER, 'validation_THRESHOLD_CLASS_class-{}'.format(class_num), 'max')
        best_thresholds[class_num-1] = best_threshold
    np.save(os.path.join(SAVE_FOLDER, 'THRESHOLD_CLASS_best_thresholds.npy'), best_thresholds)
